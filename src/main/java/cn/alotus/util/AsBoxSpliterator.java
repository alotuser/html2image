...
public class AsBoxSpliterator implements Spliterator<Box> {
	...
	private enum State {
		BOX_CHILDREN, BLOCK_INLINE_CONTENT, ILB_INLINE_CHILDREN, DONE;
	}

	public AsBoxSpliterator(Box parent) {
        unprocessed.add(parent);
    }

	private boolean hasChildren(Box bx) {
		
		if(bx.getChildCount() > 0) {
			return true;
		}
		
		if(bx instanceof BlockBox) {
			BlockBox  bb= (BlockBox) bx;
			if(bb.getInlineContent()!=null&&!bb.getInlineContent().isEmpty()) {
				return true;
			}
			
		}else if(bx instanceof InlineLayoutBox ) {
			InlineLayoutBox bb= (InlineLayoutBox) bx;
			if(bb.getInlineChildCount() > 0) {
				return true;
			}
		}
		return  false;
